...
from studies.bpgs_study.common.paths import PROJECT_ROOT, SUBSET_ROOT
from studies.bpgs_study.common.runtime import Stage
from studies.bpgs_study.common.specs import OverrideSpec, TrainingVariant


def build_subset_stage(study_name: str) -> Stage:
    """Build a stage that generates NYUv2 subset files."""
    script = PROJECT_ROOT / "studies" / "bpgs_study" / "nyuv2_subsets.py"
    return Stage(
        label=f"{study_name} :: prepare_nyuv2_subsets",
...
def build_training_stage(
    ...
    The ablation runner owns this logic entirely.  Overrides are
