...
from studies.bpgs_study.real_data.commands import build_training_stage
from studies.bpgs_study.real_data.utils import CONFIG_DIR, CONFIG_FILES


class RealDataRunner:
    """Owns all execution planning for real-data studies.

    The real_data runner has full control over:
    - how training commands are built (via its own commands.py)
    - which overrides apply to which variants (method-scoping)
    - real-data-specific validation
    """
    ...
    def plan(self, study: StudySpec, request: RunRequest) -> list[Stage]:
        if study.kind != "training":
            raise TypeError(
                f"Real-data study '{study.name}' must be a training study, "
        ...
        for variant in select_variants(study, request.variant_labels):
            if not isinstance(variant, TrainingVariant):
                raise TypeError(
                    f"Real-data variant '{variant.label}' must be a TrainingVariant."
                )
