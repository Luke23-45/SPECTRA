...
class BPGSScaler(nn.Module):
    ...
    def auto_calibrate(self, losses: torch.Tensor) -> None:
        ...
        # [SOTA TITANIUM FIX] Double-sided structural clamp to prevent log(0) [NaN] and log(Inf) [Inf].
        target_magnitude = safe_losses.clamp(min=1e-4)
        target_s = torch.log(target_magnitude)
    ...
    def forward(
        ...
        # [SOTA TITANIUM FIX] fp16 Shielding: Cast to float32 before clone/all_reduce 
        # to prevent silent overflow during distributed summation mapping.
        losses_for_ema = losses.detach().float()
        if sync_ddp and dist.is_initialized():
        ...
        with torch.no_grad():
            # [SOTA TITANIUM FIX] NaN-Gate EMA Protection
            # If a batch produces an Inf/NaN loss, reject the EMA update to prevent poisoning.
            if torch.isfinite(losses_for_ema).all():
                ...
                logger.warning("[B-PGS] NaN-Gate Triggered: Rejected corrupt batch from EMA tracker.")
        ...
        # Removed: total_loss = total_loss.to(losses.dtype)
        # We keep total_loss in fp32. PL's GradScaler handles scaling correctly
        # and downcasting prematurely can cause overflows in early training.
